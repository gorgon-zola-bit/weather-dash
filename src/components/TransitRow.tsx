  ...
  return (
    <div className="rounded-xl p-2 bg-card-light dark:bg-card-dark border border-border-light dark:border-border-dark flex items-center justify-between gap-2">
      <div className="min-w-0">
        ...
        <div className="text-[0.75rem] text-muted-light dark:text-muted-dark">
          {route.direction} — {route.stopName}
        </div>
      </div>
      <div className="flex-shrink-0 text-right">
        {route.error ? (
          <span className="text-[0.85rem] text-amber-500">{route.error}</span>
        ) : route.arrivals.length === 0 ? (
          <span className="text-[0.85rem] text-muted-light dark:text-muted-dark">
            No arrivals
          </span>
        ) : (
          <div className="flex gap-2 items-baseline">
            {route.arrivals.map((a, i) => (
              <span key={i} className="inline-flex items-baseline gap-0.5">
                {i > 0 && (
                  <span className="text-muted-light dark:text-muted-dark text-[0.8rem] mr-1">
                    ,
                  </span>
                )}
                <span
                  className={`font-bold tabular-nums ${
                    i === 0
                      ? 'text-[1.6rem] text-primary'
                      : 'text-[1.2rem] text-text-light dark:text-text-dark'
                  }`}
                >
                  {a.minutes}
                </span>
                <span className="text-[0.7rem] text-muted-light dark:text-muted-dark">
                  min
                </span>
              </span>
            ))}
